    ...
    void OnDrawGizmos()
    {
        ...
        if (Selection.transforms.Length == 2)
        {
            ...
            if (tran1 != null && tran2 != null)
            {
                ...
                Handles.Label((tran1.position + tran2.position) / 2, $"{(tran1.position - tran2.position).magnitude * 100.0f:F3}", new GUIStyle()
                {
                    fontSize = 9
                });
            }
        ...
        if (Selection.transforms.Length == 1)
        {
            ...
            { //如果有子级(子级个数大于1)
                for (int i = 0; i < tranRoot.childCount - 1; i++)
                {
                    ...
                    if (tran1 != null && tran2 != null)
                    {
                        ...
                        Handles.Label((p1 + p2) / 2, $"{(p1 - p2).magnitude * 100.0f:F3}", new GUIStyle()
                        {
                            fontSize = 9
                        });
                    }
            ...
            { //如果没有子级
                ...
                Handles.Label(tranRoot.position / 2, $"{tranRoot.position.magnitude * 100.0f:F3}", new GUIStyle()
                {
                    fontSize = 9
                });
            }
